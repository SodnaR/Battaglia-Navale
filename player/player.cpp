...
bool Player::shot(std::shared_ptr<Ship> bship, std::string tile, Player &opposite){
    ...
    if(target != ' '){
        ...
        if(dummy->getArmor() == 0){
            for(int i = 0; i < d_grid.getMapSize(); i++){
                for(int j = 0; j < d_grid.getMapSize(); j++){
                    if(target == dummy->getId()){
                        std::string spot = toTile(i, j);
                        if(opposite.shipLegend()[spot] == dummy){
                            a_grid.setTile(spot, 'X');
                        }
                    }
                }
            }
